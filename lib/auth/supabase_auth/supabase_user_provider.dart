...
class VolkswagenConnectSupabaseUser extends BaseAuthUser {
  VolkswagenConnectSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> volkswagenConnectSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = VolkswagenConnectSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
